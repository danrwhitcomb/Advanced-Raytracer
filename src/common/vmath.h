...
inline float bernstein(float t, int i, int deg) {
    if(i < 0 or i > deg) return 0;
    if(deg == 0) return 1;
    if(deg == 1) {
        if(i == 0) return 1-t;
        if(i == 1) return t;
    ...
    return (1-t)*bernstein(t, i, deg-1) + t*bernstein(t, i-1, deg-1);
}

inline float bernstein_derivative(float t, int i, int dim) {
    return dim * (bernstein(t, i-1, dim-1) - bernstein(t, i, dim-1));
}
...
inline bool operator!=(const vec3f& a, const vec3f& b) { return a.x!=b.x or  a.y!=b.y or  a.z!=b.z; }
...
inline vec3f abs(const vec3f& a) { return vec3f(fabs(a.x),fabs(a.y),fabs(a.z)); }
...
// transform an element from a local frame to a global frame, where local is defined wrt to global
...
// transform a direction by a frame
...
inline vec3f transform_point_from_local(const frame3f& f, const vec3f& v) { return f.o + f.x * v.x + f.y * v.y + f.z * v.z; }
inline vec3f transform_vector_from_local(const frame3f& f, const vec3f& v) { return f.x * v.x + f.y * v.y + f.z * v.z; }
inline vec3f transform_direction_from_local(const frame3f& f, const vec3f& v) { return transform_vector_from_local(f,v); }
inline vec3f transform_normal_from_local(const frame3f& f, const vec3f& v) { return transform_vector_from_local(f,v); }
inline frame3f transform_frame_from_local(const frame3f& f, const frame3f& v) { return frame3f(transform_point_from_local(f,v.o), transform_vector_from_local(f,v.x), transform_vector_from_local(f,v.y), transform_vector_from_local(f,v.z)); }

// frame-element inverse transforms ------------------
// transform an element from a global frame to a local frame, where local is defined wrt to global
...
inline vec3f transform_point_to_local(const frame3f& f, const vec3f& v) { return vec3f(dot(v-f.o,f.x), dot(v-f.o,f.y), dot(v-f.o,f.z)); }
inline vec3f transform_vector_to_local(const frame3f& f, const vec3f& v) { return vec3f(dot(v,f.x), dot(v,f.y), dot(v,f.z)); }
inline vec3f transform_direction_to_local(const frame3f& f, const vec3f& v) { return transform_vector_to_local(f,v); }
inline vec3f transform_normal_to_local(const frame3f& f, const vec3f& v) { return transform_vector_to_local(f,v); }
inline frame3f transform_frame_to_local(const frame3f& f, const frame3f& v) { return frame3f(transform_point_to_local(f,v.o), transform_vector_to_local(f,v.x), transform_vector_to_local(f,v.y), transform_vector_to_local(f,v.z)); }
